...
def make_prob_list(n, deck_size=60, hand_size=5, num_specials=3, intro=""):
    ...
    def f(x):
        return 1 if x == 0 else reduce(mul, range(1, x + 1))
    ...
    base += f"({hand_size} choose {n}) = {value:.4f}"
    return base, value
...
def p_n_stars(n):
    ...
    """
    base, value = make_prob_list(n, intro="P({}\u2606) = ")
    return base, value
...
def exact_prob():
    ...
    output += f"P({d}\u2662) = "
    output += " + ".join(base.format(i) for i in range(4))
    output += "\n\n"
    ...
    out = ""
    value = 0.0
    for d_val, s_val in zip(star_values, diamond_values, strict=True):
        out += f"{d_val:.4f} * {s_val:.4f} + "
        value += d_val * s_val

    output += "{}) = {:.4f}\n".format(out.removesuffix(" + "), 1 - value)
    return output
...
def make_solution():
    ...
    output += f"this is ~= to the simulation value of: {sim:.4f}"
